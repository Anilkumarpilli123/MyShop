...
const products = [
  ...
  { id: 3, name: "Puma Classic Sneakers", price: "$35", image: shoes, category: "shoes" },
  { id: 4, name: "Nike Speed", price: "$30", image: shoes3, category: "shoes" },
  { id: 5, name: "Nike Air Max", price: "$28", image: shoes4, category: "shoes" },
...
// }, []);
  ...
  return (
    <section
      className="hero"
      style={{
        backgroundImage: loadedImages.length > 0 ? `url(${banners[currentIndex]})` : 'none',
...
function ProductSlider({ title, items, sliderId }) {
 ...
 const { addToWishlist, removeFromWishlist, isInWishlist } = useWishlist();
  ...
  React.useEffect(() => {
    ...
    const updateButtons = () => {
      ...
      setShowRight(slider.scrollLeft < maxScroll - 5);
    };
  ...
  return (
    <section className="featured">
      ...
      <h3>{title}</h3>
      <Link to={`/category/${items[0]?.category}`} className="see-more-link">
      See More →
    </Link>
    </div>
      <div className="slider-products">
        {showLeft && (
          <button className="slider-button left" onClick={() => scrollSlider("left")}>
            ❮
          </button>
        )}
        <div className="products-slider" id={sliderId} ref={sliderRef}>
          {items.map((product) => (
            <div key={product.id} className="product-card">
              ...
              <Link to={`/products/${product.id}`} className="product-info-link">
                {/* <img src={product.image} alt={product.name} className="product-image" /> */}
                <LazyLoadImage src={product.image} alt={product.name} className="product-image" 
                // effect="blur"
                width="100%"
                height="auto"
                />
              ...
              <button onClick={() => addToCart(product)} disabled={isInCart(product.id)}>
        ...
        {showRight && (
          <button className="slider-button right" onClick={() => scrollSlider("right")}>
            ❯
          </button>
        )}
      </div>
...
function HomePage({ searchQuery }) {
  ...
  ? products.filter((product) => product.category.toLowerCase().includes(normalizedQuery))
  : products;

  const shoes = filteredProducts.filter((product) => product.category === "shoes");
  const shirts = filteredProducts.filter((product) => product.category === "shirts");
  const jackets = filteredProducts.filter((product) => product.category === "jacket");
  const hoodies = filteredProducts.filter((product) => product.category === "hoodies");
  const handbags = filteredProducts.filter((product) => product.category === "handbag");
  const mobiles = filteredProducts.filter((product) => product.category === "mobile");
  const laptops = filteredProducts.filter((product) => product.category === "laptops");
  const smartwatchs = filteredProducts.filter((product) => product.category === "smartwatchs");
  const smartTvs = filteredProducts.filter((product) => product.category === "smartTvs");
  const actiontoys = filteredProducts.filter((product) => product.category === "actiontoys");
  const remotecontroltoys = filteredProducts.filter((product) => product.category === "remotecontroltoys");
  const softteddytoys = filteredProducts.filter((product) => product.category === "softteddytoys");
  const fictionbooks = filteredProducts.filter((product) => product.category === "fictionbooks");
  const personaldevelopmentbooks = filteredProducts.filter((product) => product.category === "personaldevelopmentbooks");
  const educationalbooks = filteredProducts.filter((product) => product.category === "educationalbooks");
  ...
  return (
    <div>
      ...
      {/* Sliders */}
      {shoes.length > 0 && (
  <ProductSlider title="Featured Shoes" items={shoes} sliderId="shoesSlider" />
)}

{shirts.length > 0 && (
  <ProductSlider title="Featured Shirts" items={shirts} sliderId="shirtsSlider" />
)}

{jackets.length > 0 && (
  <ProductSlider title="Featured Jackets" items={jackets} sliderId="jacketsSlider" />
)}

{hoodies.length > 0 && (
  <ProductSlider title="Featured Hoodies" items={hoodies} sliderId="hoodiesSlider" />
)}

{handbags.length > 0 && (
  <ProductSlider title="Featured Handbags" items={handbags} sliderId="handbagSlider" />
)}

{mobiles.length > 0 && (
  <ProductSlider title="Featured Mobiles" items={mobiles} sliderId="mobileSlider" />
)}

{laptops.length > 0 && (
  <ProductSlider title="Featured Laptops" items={laptops} sliderId="laptopSlider" />
)}

{smartwatchs.length > 0 && (
  <ProductSlider title="Featured Smart Watches" items={smartwatchs} sliderId="smartwatchSlider" />
)}

{smartTvs.length > 0 && (
  <ProductSlider title="Featured Smart Tvs" items={smartTvs} sliderId="smarttvSlider" />
)}

{actiontoys.length > 0 && (
  <ProductSlider title="Featured Action Figures" items={actiontoys} sliderId="actiontoySlider" />
)}

{remotecontroltoys.length > 0 && (
  <ProductSlider title="Featured Remote Control Toys" items={remotecontroltoys} sliderId="remotecontroltoySlider" />
)}

{softteddytoys.length > 0 && (
  <ProductSlider title="Featured Soft Toys" items={softteddytoys} sliderId="softtoysSlider" />
)}

{fictionbooks.length > 0 && (
  <ProductSlider title="Featured Fiction Books" items={fictionbooks} sliderId="fictionbookSlider" />
)}

{personaldevelopmentbooks.length > 0 && (
  <ProductSlider title="Featured Personal Development Books" items={personaldevelopmentbooks} sliderId="personaldevelopmentbookSlider" />
)}

{educationalbooks.length > 0 && (
  <ProductSlider title="Featured Educational Books" items={educationalbooks} sliderId="educationbookSlider" />
)}
{shoes.length === 0 && shirts.length === 0 && jackets.length === 0 && hoodies.length === 0 && handbags.length === 0 && mobiles.length === 0 && laptops.length === 0 && smartwatchs.length === 0 && smartTvs.length === 0 &&
 actiontoys.length === 0 && remotecontroltoys.length === 0 && softteddytoys.length === 0 && fictionbooks.length === 0 && personaldevelopmentbooks === 0 && educationalbooks.length === 0 && (
